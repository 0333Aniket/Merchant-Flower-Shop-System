<%@page import="com.Entity.adduser"%>
...
<li class="item">
  ...
  <form class="form-inline" action="search_customer" method="post">
  ...
  <tbody>
    ...
    <%
      try{
    	  ...
    	  List<adduser> li=pm.get_user(fullName);
    	  for(adduser ad:li){
    ...
    %>
      ...
      <td><%=ad.getContact() %></td>
       <td><%=ad.getName() %></td>
      <td><%=ad.getType() %></td>
      <td scope="row"><%=ad.getTotal() %></td>
       <td><%=ad.getAdvance() %></td>
      <td><%=ad.getRemaining() %></td>
    </tr>
    <%
    	  }}catch(Exception e){
    		  e.printStackTrace();
